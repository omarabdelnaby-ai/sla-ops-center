  ...
  if (action === "forecast") {
    const hourly = HOURS.map((time, i) => {
      ...
      return {
        time, hour: 9 + i,
        chat: {
          sla: chatIRT3[i], actVol: chatVol[i],
          fteActual: chatFTE[i], fteReq: chatReq[i],
          fteGap: cGap, backlog: chatBL[i],
          occupancy: chatOcc[i], riskScore: cRS,
          riskLabel: riskLabel(cRS),
          recommendation: rec(cRS, cGap, chatBL[i], "chat"),
        },
        email: {
          sla: emailRT2[i], actVol: emailVol[i],
          fteActual: emailFTE[i], fteReq: emailReq[i],
          fteGap: eGap, backlog: emailBL[i],
          occupancy: emailOcc[i], riskScore: eRS,
          riskLabel: riskLabel(eRS),
          recommendation: rec(eRS, eGap, emailBL[i], "email"),
        },
        phone: {
          sla: phoneSLA[i], actVol: phoneVol[i],
          fteActual: phoneFTE[i], fteReq: phoneReq[i],
          fteGap: pGap, backlog: 0,
          occupancy: phoneOcc[i], riskScore: pRS,
          riskLabel: riskLabel(pRS),
          recommendation: rec(pRS, pGap, 0, "phone"),
        },
        overallRisk: avg,
  ...
  if (action === "insights") {
    return {
      insights: [{
        ...
        type: "Multi-Channel SLA Analysis",
        text: "CHAT: IRT 3min avg 56% — Hours 9AM and 6PM-10PM at risk due to zero staffing.\n\nEMAIL: RT 2hr avg 67% — Backlog building from 4PM onwards. Pre-position 2 FTE before 4PM.\n\nPHONE: SLA strong 10AM-5PM at 100%. Evening hours unstaffed.\n\nTop 3 actions:\n1. Review Chat staffing 9AM — only 6 FTE vs 22 required\n2. Clear Email backlog before 4PM each day\n3. Confirm Phone evening coverage requirement",
      }],
    };
  }
