export function transformForecastData(raw) {
    ...
    if (raw.hourly && Array.isArray(raw.hourly)) {
      for (const entry of raw.hourly) {
        ...
        if (entry.time === 'Total' || entry.time === 'Day' || entry.time === 'Night') continue;
      ...
      if (raw.chat && Array.isArray(raw.chat)) channels.chat = raw.chat.filter(r => r.hour >= 9 && r.hour <= 21).map((r, i) => parseEntry(r, r.hour || 9 + i));
      if (raw.email && Array.isArray(raw.email)) channels.email = raw.email.filter(r => r.hour >= 9 && r.hour <= 21).map((r, i) => parseEntry(r, r.hour || 9 + i));
      if (raw.phone && Array.isArray(raw.phone)) channels.phone = raw.phone.filter(r => r.hour >= 9 && r.hour <= 21).map((r, i) => parseEntry(r, r.hour || 9 + i));
    }
    return channels;
  }
